...
async function run() {
  try {
    ...
    console.log("Searching for sale AZB-VENTA-2026-0013...");
    const { data: sales, error: saleErr } = await supabase
      ...
      .select(`
        *,
        clientes (*)
      `)
      .eq("codigo_venta", "AZB-VENTA-2026-0013");
    ...
    if (!sales || sales.length === 0) {
      console.log("No sale found with code AZB-VENTA-2026-0013.");
      return;
    }
    ...
    logs.forEach(log => {
      console.log(`[${log.creado_en}] [${log.integracion}] [${log.tipo}] ${log.mensaje}`);
    });
