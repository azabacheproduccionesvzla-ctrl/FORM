...
export default function ImportModal({ isOpen, onClose, onSuccess }: ImportModalProps) {
    ...
    reader.onload = (e) => {
      try {
        ...
        const mapped: ParsedRow[] = rawJson.map((row) => {
          ...
          const tipoProyecto = (row["Tipo Proyecto"] || row["Project Type"] || "Precio Fijo").toString().trim();
          const statusPago = (row["Status Pago"] || row["Payment Status"] || "Pago Adelantado").toString().trim();
          const setterUsername = (row["Setter"] || row["Vendedor"] || "").toString().trim();
          const closerUsername = (row["Closer"] || "").toString().trim();
          ...
          if (!projectName) {
            isValid = false;
            errorMsg = "Falta el nombre del proyecto. ";
          }

          return {
            proyecto_nombre: projectName,
            proyecto_link: projectLink,
            proyecto_brief: brief,
            monto_total: isNaN(montoTotal) ? 0 : montoTotal,
            moneda,
            plataforma,
            ...
            closer_username: closerUsername,
            isValid,
            errorMsg
          };
        });
  ...
  const handleDownloadTemplate = () => {
    // Generate template
    const headers = [
      "Proyecto", "Link Proyecto", "Brief", "Monto Total", "Moneda", "Plataforma",
      "Tipo Venta", "Tipo Proyecto", "Status Pago", "Setter", "Closer"
    ];
    const exampleRow = [
      "Diseño de Landing Page", "https://workana.com/job/123", "https://drive.google.com/brief", "450", "USD", "Workana",
      "Nueva Venta", "Precio Fijo", "Pago Adelantado", "VictoriaR", "ArgenisF"
    ];

    const ws = XLSX.utils.aoa_to_sheet([headers, exampleRow]);
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, "Plantilla Importación Proyectos");
    
    // Write and trigger download
    XLSX.writeFile(wb, "Plantilla_Importacion_Proyectos.xlsx");
  };
          ...
          <div>
            ...
            <div style={{ backgroundColor: "#eff6ff", border: "1px solid #bfdbfe", borderRadius: "8px", padding: "1rem", fontSize: "0.85rem", color: "#1e3a8a" }}>
              ...
              <p style={{ margin: "0 0 0.75rem 0", lineHeight: "1.4" }}>
                El archivo debe contener columnas con encabezados como: <strong>Proyecto</strong> (Nombre), <strong>Monto Total</strong>, <strong>Moneda</strong>, <strong>Plataforma</strong>, <strong>Tipo Venta</strong>, <strong>Setter</strong> y <strong>Closer</strong>. Todos los proyectos importados serán asociados por defecto al cliente <strong>Cliente Trello Sin Clasificar</strong>.
              </p>
              <button
                type="button"
              ...
              <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "0.8rem", textAlign: "left" }}>
                <thead style={{ position: "sticky", top: 0, backgroundColor: "#f8fafc", zIndex: 1, borderBottom: "2px solid #e2e8f0" }}>
                  <tr>
                    ...
                    <th style={{ padding: "0.5rem 0.75rem", fontWeight: "700", color: "#475569" }}>Proyecto</th>
                    <th style={{ padding: "0.5rem 0.75rem", fontWeight: "700", color: "#475569" }}>Monto</th>
                    <th style={{ padding: "0.5rem 0.75rem", fontWeight: "700", color: "#475569" }}>Plataforma</th>
                    <th style={{ padding: "0.5rem 0.75rem", fontWeight: "700", color: "#475569" }}>Setter / Closer</th>
                  </tr>
                </thead>
                <tbody>
                  {parsedRows.map((row, idx) => (
                    <tr key={idx} style={{ borderBottom: "1px solid #e2e8f0", backgroundColor: row.isValid ? undefined : "#fff5f5" }}>
                      ...
                      <td style={{ padding: "0.5rem 0.75rem", color: "#475569" }}>
                        {row.setter_username || "—"} / {row.closer_username || "—"}
                      </td>
                    </tr>
                  ))}
                </tbody>
          ...
          <div>
            <div style={{ textAlign: "center", padding: "1.5rem 0" }}>
              ...
              <p style={{ fontSize: "0.9rem", color: "#64748b", margin: 0, maxWidth: "450px", marginLeft: "auto", marginRight: "auto" }}>
                Los proyectos han sido importados. Se crearon los registros de ventas correspondientes vinculados al cliente por defecto <strong>Cliente Trello Sin Clasificar</strong>. Las automatizaciones se están ejecutando en segundo plano.
              </p>
            </div>

            <div style={{ display: "grid", gridTemplateColumns: "1fr", gap: "1rem", marginBottom: "1.5rem" }}>
              <div style={{ backgroundColor: "#f0f5ff", border: "1px solid #bfdbfe", borderRadius: "8px", padding: "1rem", textAlign: "center" }}>
                <span style={{ fontSize: "0.85rem", color: "#1e40af" }}>Proyectos/Ventas Registrados</span>
                <h4 style={{ fontSize: "1.75rem", margin: "0.25rem 0 0 0", color: "#1d4ed8", fontWeight: "700" }}>{importResult.importedSales}</h4>
              </div>
            </div>
        ...
        <div className={styles.loadingOverlay} style={{ zIndex: 1200 }}>
          <div className={styles.loadingOverlayContent} style={{ maxWidth: "350px", textAlign: "center" }}>
            ...
            <span className={styles.loadingTextOverlay} style={{ fontSize: "0.8rem", opacity: 0.85 }}>
              Insertando ventas en base de datos y disparando integraciones con Trello, Dropbox y GoHighLevel. Por favor espera...
            </span>
          </div>
        </div>
      )}
